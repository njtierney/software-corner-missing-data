...
```{r}
#| label: table-rodents-slice
#| echo: false
rodents %>% 
  # selected snapshot of interesting missingness
  filter(between(row_number(), 75, 85)) %>% 
...
The syntax for the other numerical summaries in `naniar` are `miss_`, and then `case`, or `var` to refer to cases or variables. There are then `summary`, `table` suffixes.
...
There are also summary functions for exploring missings that occur over a particular span or period of the dataset, or the number of missings in a single run - `miss_var_run()`, and `miss_var_span()`. The reader can see the use of these functions in the documentation: https://naniar.njtierney.com/reference/miss_var_run.html and  https://naniar.njtierney.com/reference/miss_var_span.html 

`miss_var_run()` provides summaries for the number of missings or complete values in a single "run". This is particularly useful for time series data. We demonstrate `miss_var_run()` with the built-in dataset, `pedestrian`, containing hourly counts of pedestrians from four locations around Melbourne, Australia, from 2016. We explore the runs of missingness for `hourly_counts` like so:
...
To instead explore the number of missings over a repeating span of rows, you can use `miss_var_span()`. Below we specify the span for `hourly_counts` and then the size of the span with `span_every`.
...
This displays the number of missing values in each variable. We learn similar information to Figure @fig-vis-miss: total length, and ear length have the most missing values, followed by tail length, hind foot length, weight, and age. Just like with `vis_miss()`, we can add in facets in these plots, via the `facet` argument, see @fig-gg-miss-var-facet. We learn again, that the species have similar amounts of missing data in their variables. While this confirms the same piece of information, it is useful.
...
It feels like there are several patterns with the missingness - some variables tend to go missing at the same time. To explore these patterns we can use `gg_miss_upset()`, which produces an "upset" plot [@Conway2017] of the intersecting sets of missingness. This can be thought of as a generalised way to visualise intersecting venn diagrams.
...
As evidenced by @gg-miss-upset, there is a structure in the missingness in the rodents data. We can perform some basic clustering on the missingness and then and learn which variables and their values predict these missingness groups using decision trees [@Tierney2015; @Barnett2017]. We start by adding missingness clusters, choosing four based on @gg-miss-upset. We encourage exploring different numbers of clusters. We can then confirm this pattern using visualisations.
...
Variable importance scores (@tab-var-imp) reveal the most important variables for predicting missingness cluster are date and sex. 
...
Ah, we learn something important here - there aren't many imputations at all! This is because we are imputing tail length with weight and age, which are also missing at similar rates. We can check this by doing just a plot using `vis_miss()` with only tail length, weight, and age.
...
We can resolve this by first imputing weight and age separately, then imputing tail length from weight and age.
